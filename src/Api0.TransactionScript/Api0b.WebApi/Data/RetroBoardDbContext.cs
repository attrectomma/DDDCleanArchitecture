...
namespace Api0b.WebApi.Data;

/// <summary>
/// EF Core DbContext for the Api0b (Transaction Script + Concurrency Safety)
/// retro board database.
/// </summary>
/// <remarks>
/// DESIGN: Same inline <see cref="OnModelCreating"/> approach as Api0a, with
/// one addition: <c>xmin</c> concurrency token configuration on
/// <see cref="User"/>, <see cref="Project"/>, and <see cref="RetroBoard"/>.
///
/// The <c>xmin</c> column is a PostgreSQL system column that stores the
/// transaction ID of the last write. EF Core uses it as a concurrency token
/// by adding <c>WHERE xmin = @expected</c> to UPDATE statements. If another
/// transaction modified the row since we read it, the WHERE clause matches
/// zero rows and EF Core throws <c>DbUpdateConcurrencyException</c>.
///
/// Compare with API 3+ where xmin is configured on aggregate roots in
/// separate <c>IEntityTypeConfiguration</c> classes. Same mechanism,
...
public class RetroBoardDbContext : DbContext
{
    ...
    /// Includes xmin concurrency tokens on User, Project, and RetroBoard.
    /// </summary>
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<User>(builder =>
        {
            ...
            // DESIGN: xmin concurrency token — detects concurrent modifications.
            // This is the ONLY change from Api0a's User configuration.
        ...
        modelBuilder.Entity<Project>(builder =>
        {
            ...
            // DESIGN: xmin concurrency token — same pattern as User.
        ...
        modelBuilder.Entity<RetroBoard>(builder =>
        {
            ...
            // DESIGN: xmin concurrency token — same pattern as User/Project.
