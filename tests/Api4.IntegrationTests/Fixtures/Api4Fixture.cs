...
namespace Api4.IntegrationTests.Fixtures;

/// <summary>
/// API 4-specific fixture that configures WebApplicationFactory
/// to use a Testcontainer Postgres database and applies EF Core migrations.
/// </summary>
/// <remarks>
/// DESIGN: Same pattern as Api1Fixture/Api2Fixture/Api3Fixture — this fixture
/// manages its own <see cref="PostgresFixture"/> internally. The only difference
/// is the DbContext type (<see cref="RetroBoardDbContext"/> from Api4.Infrastructure)
/// and the interceptor type.
/// </remarks>
public class Api4Fixture : ApiFixture<Program>
{
    private readonly PostgresFixture _ownedPostgresFixture;

    /// <summary>
    /// Initializes a new instance of <see cref="Api4Fixture"/>.
    /// Creates and manages its own <see cref="PostgresFixture"/>.
    /// </summary>
    public Api4Fixture() : this(new PostgresFixture())
    {
    }

    private Api4Fixture(PostgresFixture postgresFixture) : base(postgresFixture)
    {
    ...
    protected override void ConfigureServices(IServiceCollection services)
    {
        ...
        services.AddDbContext<RetroBoardDbContext>((sp, options) =>
        {
            ...
            options.AddInterceptors(
                sp.GetRequiredService<Api4.Infrastructure.Persistence.Interceptors.AuditInterceptor>());
        });
    }
